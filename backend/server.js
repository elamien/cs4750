import 'dotenv/config'; // Ensures dotenv is configured at the very top
import express from 'express';
import cors from 'cors';
import mysql from 'mysql2/promise';

const app = express();
const PORT = process.env.PORT || 3001; // Using 3001 to avoid conflict with Vue dev server (often 3000 or 5173)
...
const pool = mysql.createPool({
  ...
  user: process.env.DB_USER, // Your MySQL username from login-path or .env
  password: process.env.DB_PASSWORD, // Your MySQL password from login-path or .env
...
// Basic Routes
app.get('/', (req, res) => {
  res.send('HooJams Backend is running!');
});

// --- API Routes for Fill-In Requests ---

// GET /api/fill-in-requests - Fetch all fill-in requests with joined data
app.get('/api/fill-in-requests', async (req, res, next) => {
  try {
    const query = `
      SELECT 
    ...
    // Format dates to ISO strings if they aren't already (MySQL TIMESTAMPS are usually fine)
    const formattedRows = rows.map(row => ({
      ...
      // Convert INT ids from DB to string for frontend consistency, if needed by frontend interfaces
      id: String(row.id),
...
app.post('/api/fill-in-requests/:id/accept', async (req, res, next) => {
  const { id: fillInRequestId } = req.params;
  const { userId: acceptedByUserId } = req.body; // Expecting userId in the request body
  ...
  try {
    ...
    // Optional: Check if the user accepting is the original member who posted it
...
// TODO: Add API routes for users, bands, events etc.
